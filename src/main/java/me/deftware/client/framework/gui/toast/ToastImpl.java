...
public class ToastImpl implements IToast {
	...
	public Visibility draw(GuiToast manager, long startTime) {
		...
		// Title
		manager.getMinecraft().fontRenderer.drawString(title.toString(true), 30.0F, 7.0F, -1);
		...
		if (text != null && text.length != 0) {
			...
			manager.getMinecraft().fontRenderer.drawString(text[index].toString(true), 30.0F, 18f, -1);
		}
