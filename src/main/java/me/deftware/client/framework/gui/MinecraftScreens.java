...
public enum MinecraftScreens {
	...
	Multiplayer(GuiMultiplayer::new), WorldSelection(GuiSelectWorld::new),
	Options(parent -> new GuiOptions(parent, Minecraft.getMinecraft().gameSettings)), MainMenu(parent -> new GuiMainMenu()),
	Mods(parent -> {
	...
	public void open(GuiScreen parent) {
		net.minecraft.client.Minecraft.getMinecraft().displayGuiScreen(screenFunction.apply(parent));
	}

	@SuppressWarnings("ConstantConditions")
	public static void openRealms() {
		new RealmsBridge().switchToRealms(Minecraft.getMinecraft().currentScreen);
	}
