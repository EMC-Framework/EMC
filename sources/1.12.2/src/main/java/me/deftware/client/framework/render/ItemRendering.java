...
public class ItemRendering {
	...
	public void drawItemStack(GLX context, ItemStack stack, int x, int y, int z) {
		render(context, z, () -> getRenderItem().renderItemAndEffectIntoGUI(null, (net.minecraft.item.ItemStack) stack, x, y));
	}
	...
	public void drawStackLabel(GLX context, ItemStack stack, int x, int y, int z, String count) {
		render(context, z, () ->
				getRenderItem().renderItemOverlayIntoGUI(Minecraft.getMinecraft().fontRenderer,
	...
	private void render(GLX ctx, int zOffset, Runnable runnable) {
		float prev = getRenderItem().zLevel;
		getRenderItem().zLevel = zOffset;
		runnable.run();
		getRenderItem().zLevel = prev;
	}
