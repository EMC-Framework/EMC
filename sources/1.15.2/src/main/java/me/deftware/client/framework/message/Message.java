...
public interface Message extends com.mojang.brigadier.Message {
    ...
    Message SPACE = (Message) new LiteralText(" ");

    Message EMPTY = (Message) new LiteralText("");
    ...
    default Optional<Message> visit(BiFunction<Appearance, String, Optional<Message>> visitor) {
        for (Text text : (Text) this) {
            Optional<Message> result = visitor.apply((Appearance) text.getStyle(), text.getString());
            if (result.isPresent()) {
                return result;
            }
        }
    ...
    default Message style(Appearance style) {
        ((Text) this).setStyle((Style) style);
        return this;
    }
    ...
    static Message of(String text) {
        return (Message) new LiteralText(text);
    }
    ...
    static Message translated(String key, Object... args) {
        return (Message) new TranslatableText(key, args);
    }
