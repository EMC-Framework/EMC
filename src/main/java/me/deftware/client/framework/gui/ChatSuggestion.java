	...
	public class Window implements Drawable {
		...
		public void onRender(int mouseX, int mouseY, float delta) {
			...
			if (text.length() > 0 && !this.suggestions.isEmpty()) {
				...
				if (end > maxItems) {
					start = MathHelper.clamp_int(this.selectedIndex, 0, this.suggestions.size() - maxItems);
		...
		private void setSelectedIndex(int offset) {
			this.selectedIndex = MathHelper.clamp_int(this.selectedIndex + offset, 0, this.suggestions.size() - 1);
		}
