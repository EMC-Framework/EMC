...
public abstract class GuiScreen extends Screen implements GenericScreen {
	...
	public void render(int mouseX, int mouseY, float partialTicks) {
		...
		GLX glx = GLX.of(new MatrixStack());
	...
	protected GuiScreen addCenteredText(int x, int y, Message text) {
		addComponent(
				new Label(x - MinecraftClient.getInstance().textRenderer.getStringWidth(text.string()) / 2, y, text)
		);
		return this;
	}
	...
	public interface BackgroundType {
		 ...
		 */
		BackgroundType Textured = (context, mouseX, mouseY, delta, parent) -> parent.renderDirtBackground(0);
		 ...
		 */
		BackgroundType TexturedOrTransparent = (context, mouseX, mouseY, delta, parent) -> parent.renderBackground(0);
